...
//Sarina Saqib 2249047
	...
	public class Hangman {
		...
		// for every instance that the letter is in the target word, the guess is correct.
		...
		public String getCurrentGuessForDisplay(String currentGuess) {
			...
			for (int i = 0; i < currentGuess.length(); i++ ) {
				if (currentGuess.charAt(i) == '*') {
					...
					input.append(currentGuess.charAt(i) + " ");
				}
		...
		//at every instance of * convert to _ with a space. at every instance its a letter convert to
		// same letter with a space. Trim is added to get rid of extra space, upper case is added
		//to output the guesses in capital letters just like hangman game.
		...
		StringBuilder newLetter = new StringBuilder(target);
		
		for (int i = 0; i < target.length(); i++) {
			if (guess.charAt(i) == '*') {
				if (letter == target.charAt(i))
					newLetter.setCharAt(i, letter);
			else 
				newLetter.setCharAt(i, '*');
		}
		}
			return newLetter.toString();
		}

		//updating letter with other correct guesses - using setCharAt
